...
### P3 - 프리셋 고급 기능 (향후)
- [ ] 프리셋 수정 기능 (Update UI)
- [ ] 프리셋 공유 (공개/비공개 설정, 공유 링크)
- [ ] AI 추천 프리셋 (Phase 5와 연동)
- [ ] 프리셋 사용 통계 (usage_count 추적)
- [ ] 프리셋 즐겨찾기 (is_favorite)
- [ ] 프리셋 복제 (시스템 프리셋 → 내 프리셋으로 복사)
...
### 데이터 수집 상태
- [ ] US 전체 데이터 수집 (현재 테스트 데이터만 있음)
- [ ] KR 전체 데이터 수집 (기술 지표 추가 후 재수집 필요)

### 백업 전략
- [ ] Google Drive 백업 구현 (rclone)
- [ ] backup.yml 워크플로우 수정
